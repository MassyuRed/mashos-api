...
"""Build or verify the exact rc0028 downstream experiment closure."""
...
from emlis_ai_rc0028_downstream_experiment_dependency_manifest_v3 import (  # noqa: E402
...
def _write_body_free_json(path: Path, value: object) -> None:
    if path.exists():
        raise Rc0028DownstreamExperimentDependencyError(
            "RC0028_DOWNSTREAM_OUTPUT_ALREADY_EXISTS"
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.output is not None:
            manifest = build_rc0028_downstream_experiment_dependency_manifest(
                parent,
                repo_root=repo_root,
            )
        ...
        issues = validate_rc0028_downstream_experiment_dependency_manifest(
            current,
        ...
        return 0
    except Rc0028DownstreamExperimentDependencyError as error:
        print(error.code, file=sys.stderr)
        return 1
    except (OSError, TypeError, ValueError):
        print("RC0028_DOWNSTREAM_TOOL_INPUT_INVALID", file=sys.stderr)
        return 1

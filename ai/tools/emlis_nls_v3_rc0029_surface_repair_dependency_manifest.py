...
"""Build or verify the exact rc0029 surface-repair delta closure."""
...
from emlis_ai_rc0029_surface_repair_experiment_dependency_manifest_v3 import (  # noqa: E402
...
def _load_exact_parent(path: Path) -> object:
    if hashlib.sha256(path.read_bytes()).hexdigest() != (
        ...
        raise Rc0029SurfaceRepairDependencyError(
            "RC0029_PARENT_MANIFEST_FILE_DRIFT"
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.output is not None:
            manifest = build_rc0029_surface_repair_dependency_manifest(
                parent,
                repo_root=repo_root,
            )
        ...
        issues = validate_rc0029_surface_repair_dependency_manifest(
            current,
        ...
        return 0
    except Rc0029SurfaceRepairDependencyError as error:
        print(error.code, file=sys.stderr)
        return 1
    except (OSError, TypeError, ValueError):
        print("RC0029_TOOL_INPUT_INVALID", file=sys.stderr)
        return 1

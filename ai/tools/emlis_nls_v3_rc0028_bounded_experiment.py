...
"""Run bounded, private-only diagnostics for the rc0028 experiment.

The baseline command preserves the frozen rc0027 default-value projection.
The E3 and E4 commands consume only the disconnected rc0028 private runtime.
...
_RC0028_UPSTREAM_DEPENDENCY_PATH = (
    _CYCLE_ROOT / "cycle001_dependency_manifest_rc0028_experiment.json"
)
_RC0028_DOWNSTREAM_DEPENDENCY_PATH = (
    _CYCLE_ROOT
    / "cycle001_dependency_manifest_rc0028_downstream_experiment.json"
)
_REPRESENTATIVE8_PATH = _CYCLE_ROOT / "rc0028_representative8_body_free.json"

_PRIVATE_FILENAME = "rc0027_default_baseline_private.json"
_BODY_FREE_FILENAME = "rc0027_default_baseline_body_free_receipt.json"
_PRIVATE_SCHEMA = (
    "cocolon.emlis.nls_v3.rc0028.rc0027_default_baseline.private.v1"
)
_BODY_FREE_SCHEMA = (
    "cocolon.emlis.nls_v3.rc0028.rc0027_default_baseline.body_free.v1"
)
_E3_PRIVATE_FILENAME = "rc0028_e3_representative8_private.json"
_E3_BODY_FREE_FILENAME = (
    "rc0028_e3_representative8_body_free_receipt.json"
)
_E4_PRIVATE_FILENAME = "rc0028_e4_frozen100_private.json"
_E4_BODY_FREE_FILENAME = "rc0028_e4_frozen100_body_free_receipt.json"
_E3_PRIVATE_SCHEMA = (
    "cocolon.emlis.nls_v3.rc0028.e3_representative8.private.v1"
)
_E3_BODY_FREE_SCHEMA = (
    "cocolon.emlis.nls_v3.rc0028.e3_representative8.body_free.v1"
)
_E4_PRIVATE_SCHEMA = (
    "cocolon.emlis.nls_v3.rc0028.e4_frozen100.private.v1"
)
...
_E3_PRODUCT_READ_AUTHORIZATION_SCHEMA = (
    "cocolon.emlis.nls_v3.rc0028.e3_product_read_authorization."
    "body_free.v1"
)
...
class BoundedExperimentStop(RuntimeError):
    ...
    def __init__(self, code: str) -> None:
        if type(code) is not str or _STOP_CODE_RE.fullmatch(code) is None:
            code = "RC0028_BOUNDED_EXPERIMENT_STOP"
        self.code = code
        super().__init__(code)
...
class _ClosedArgumentParser(argparse.ArgumentParser):
    def error(self, message: str) -> None:
        del message
        raise BoundedExperimentStop("RC0028_CLI_ARGUMENT_REJECTED")
...
def _sha256_file(path: Path) -> str:
    try:
        ...
        _stop("RC0028_FROZEN_SOURCE_UNAVAILABLE")
...
def _canonical_json_bytes(value: Any) -> bytes:
    try:
        ...
        _stop("RC0028_PRIVATE_PROJECTION_NOT_CANONICAL")
    ...
    *,
    ...
    if isinstance(value, tuple) and hasattr(type(value), "_fields"):
        ...
        if (
            ...
            _stop("RC0028_PRIVATE_PROJECTION_TYPE_REJECTED")
        return {
            "kind": "named_tuple",
            "type": f"{type(value).__module__}.{type(value).__qualname__}",
            "fields": {
    ...
    if isinstance(value, Mapping):
        ...
        for key, child in value.items():
            if type(key) is not str:
                _stop("RC0028_PRIVATE_PROJECTION_KEY_REJECTED")
...
def _open_private_directory(
    ...
    *,
    ...
    if (
        ...
        _stop("RC0028_PRIVATE_DIRECTORY_PREFLIGHT_REJECTED")
    ...
    try:
        ...
        if (
            ...
            _stop("RC0028_PRIVATE_OUTPUT_NAME_REJECTED")
        for name in output_names:
            try:
                os.stat(name, dir_fd=descriptor, follow_symlinks=False)
            except FileNotFoundError:
                continue
            _stop("RC0028_PRIVATE_OUTPUT_ALREADY_EXISTS")
        return descriptor
    ...
    try:
        ...
        if (
            ...
            try:
                ...
                pass
            _stop("RC0028_PRIVATE_OUTPUT_PREFLIGHT_REJECTED")
        return descriptor
    except BoundedExperimentStop:
        raise
    ...
    try:
        ...
        for name, payload in (
            ...
            while view:
                ...
                if written < 1:
                    _stop("RC0028_PRIVATE_OUTPUT_WRITE_REJECTED")
                view = view[written:]
            os.fsync(descriptor)
...
def _load_frozen_sources() -> tuple[
    ...
    try:
        ...
        _stop("RC0028_FROZEN_SOURCE_VALIDATION_FAILED")
    ...
    if (
        ...
        _stop("RC0028_FROZEN_SOURCE_CONTRACT_MISMATCH")
    ...
    if (
        ...
        _stop("RC0028_FROZEN_SOURCE_HASH_MISMATCH")
    return samples, batch_manifest, source_closure, source_hashes


def _load_rc0028_frozen_sources() -> tuple[
    list[dict[str, Any]], dict[str, Any], str, dict[str, str]
]:
    """Validate E3/E4 inputs without treating append-only rc0028 as drift.
    ...
    successor experiment; current rc0028 experiment files are not rc0027.
    """
    ...
    try:
        ...
        upstream_dependency = load_canonical_json(
            _RC0028_UPSTREAM_DEPENDENCY_PATH
        )
        downstream_dependency = load_canonical_json(
            _RC0028_DOWNSTREAM_DEPENDENCY_PATH
        )
        downstream_issues = (
            validate_rc0028_downstream_experiment_dependency_manifest(
                downstream_dependency,
                parent_manifest=upstream_dependency,
                repo_root=REPO_ROOT,
            )
        )
    except Exception:
    ...
    source_closure = downstream_dependency.get(
        "source_dependency_closure_sha256"
    )
    if (
        type(samples) is not list
        ...
        != _RC0027_FROZEN_DEPENDENCY_MANIFEST_SHA256
        or type(upstream_dependency) is not dict
        or type(downstream_dependency) is not dict
        or downstream_issues != ()
        or type(source_closure) is not str
        or _SHA256_RE.fullmatch(source_closure) is None
    ):
        _stop("RC0028_FROZEN_SOURCE_CONTRACT_MISMATCH")
...
def _case_commitments(
    ...
    if type(rows) is not list or len(rows) != _EXPECTED_CASE_COUNT:
        _stop("RC0028_CASE_COMMITMENT_SET_INVALID")
    result: dict[str, str] = {}
    for row in rows:
...
def _load_representative8_fixture(
    *,
    ...
    if _sha256_file(_REPRESENTATIVE8_PATH) != _REPRESENTATIVE8_SHA256:
        _stop("RC0028_REPRESENTATIVE8_FIXTURE_HASH_MISMATCH")
    try:
        raw = _REPRESENTATIVE8_PATH.read_bytes()
        value = json.loads(raw.decode("utf-8", errors="strict"))
    except Exception:
        _stop("RC0028_REPRESENTATIVE8_FIXTURE_INVALID")
...
def _read_private_authorization(path: Path) -> tuple[dict[str, Any], str]:
    if (
        ...
        _stop("RC0028_E3_PRODUCT_READ_AUTHORIZATION_REJECTED")
    lexical = Path(os.path.abspath(os.fspath(path)))
    try:
        resolved = path.resolve(strict=True)
    except Exception:
...
def _capture_case_projection(
    ...
    *,
    ...
    try:
        ...
        _stop("RC0028_RC0027_ADAPTER_UNAVAILABLE")
    ...
    if (
        ...
        _stop("RC0028_SAMPLE_PROJECTION_REJECTED")
    try:
        ...
        if (
            ...
            _stop("RC0028_RC0027_PIPELINE_EXCEPTION")
        private_row = {
            "candidates": [],
            "case_id": case_id,
            "disposition": {
    ...
    if validation_issues:
        _stop("RC0028_RC0027_EXECUTION_VALIDATION_FAILED")
    ...
    if runtime_status == "selected":
        ...
        _stop("RC0028_RC0027_STATUS_UNKNOWN")
    ...
    if (
        ...
        _stop("RC0028_RC0027_DISPOSITION_INVALID")
    ...
    if (
        ...
        _stop("RC0028_RC0027_BOUNDED_CANDIDATE_SET_INVALID")
    ...
    for candidate in candidates:
        try:
            ...
            _stop("RC0028_RC0027_PROJECTION_EXCEPTION")
...
def _rc0028_fail_close_projection(
    *,
    case_id: str,
    source_case_commitment: str,
    failure_code: str,
    ...
    if (
        ...
        _stop("RC0028_RC0028_RUNTIME_UNKNOWN_EXCEPTION")
    counts = {key: 0 for key in _RC0028_BODY_FREE_COUNT_KEYS}
    hashes = {key: None for key in _RC0028_BODY_FREE_HASH_KEYS}
...
def _run_rc0028_case_projection(
    sample: Mapping[str, Any],
    *,
    source_case_commitment: str,
    source_closure: str,
    ...
    try:
        from emlis_ai_step11_rc0028_experiment_runtime_adapter_v3 import (
            Step11Rc0028ExperimentRuntimeError,
        ...
        _stop("RC0028_RC0028_PRIVATE_RUNTIME_UNAVAILABLE")
    ...
    if (
        ...
        _stop("RC0028_SAMPLE_PROJECTION_REJECTED")
    try:
        execution = execute_step11_rc0028_experiment_private(
            dict(current_input),
            case_id=case_id,
            source_case_commitment=source_case_commitment,
            source_dependency_closure_sha256=source_closure,
    ...
    except Step11Rc0028ExperimentRuntimeError as exc:
        return _rc0028_fail_close_projection(
            case_id=case_id,
            source_case_commitment=source_case_commitment,
    ...
    if validate_step11_rc0028_experiment_private_execution(execution):
        _stop("RC0028_RC0028_PRIVATE_EXECUTION_INVALID")
    result = execution.body_free_result
    if validate_step11_rc0028_experiment_result(result):
        _stop("RC0028_RC0028_BODY_FREE_RESULT_INVALID")
    ...
    if (
        ...
        _stop("RC0028_RC0028_SOURCE_COMMITMENT_MISMATCH")

    counts = {
        key: getattr(result, key) for key in _RC0028_BODY_FREE_COUNT_KEYS
    }
    hashes = {
        key: getattr(result, key) for key in _RC0028_BODY_FREE_HASH_KEYS
    }
    ...
    if result.disposition == "selected":
        if type(selected_bytes) is not bytes or not selected_bytes:
            _stop("RC0028_PRODUCT_READ_INPUT_MISSING")
        product_read_input = {
        ...
        if selected_bytes is not None:
            _stop("RC0028_PRODUCT_READ_INPUT_DISPOSITION_MISMATCH")
        product_read_input = {
            "selected_final_utf8_bytes": None,
            "status": "not_selected",
        }
...
def validate_rc0028_experiment_body_free_receipt(
    value: Any,
    *,
    expected_stage: str,
) -> tuple[str, ...]:
    ...
    if expected_stage == "E3_representative8":
        ...
        return ("RC0028_RECEIPT_STAGE_INVALID",)
    issues: set[str] = set()
    if type(value) is not dict or set(value) != _RC0028_RECEIPT_TOP_KEYS:
        return ("RC0028_RECEIPT_SHAPE_INVALID",)
    ...
    if (
        ...
        issues.add("RC0028_RECEIPT_CONTRACT_INVALID")
    if (
        ...
        issues.add("RC0028_RECEIPT_ACCOUNTING_INVALID")
    if type(cases) is not list or len(cases) != expected_count:
        issues.add("RC0028_RECEIPT_CASE_SET_INVALID")
    ...
    for row in cases:
        if type(row) is not dict or set(row) != {
            ...
            issues.add("RC0028_RECEIPT_CASE_SHAPE_INVALID")
            continue
        case_id = row.get("case_id")
        ...
        if (
            ...
            issues.add("RC0028_RECEIPT_CASE_ID_INVALID")
            continue
        seen.append(case_id)
        ...
        if (
            ...
            issues.add("RC0028_RECEIPT_CLOSED_CODE_INVALID")
        if (
            type(row_counts) is not dict
            or tuple(sorted(row_counts)) != _RC0028_BODY_FREE_COUNT_KEYS
            or any(
                type(count) is not int or type(count) is bool or count < 0
            ...
            issues.add("RC0028_RECEIPT_COUNT_INVALID")
        if (
            type(row_hashes) is not dict
            or tuple(sorted(row_hashes)) != _RC0028_BODY_FREE_HASH_KEYS
            or not _valid_nonzero_sha256(
            ...
            issues.add("RC0028_RECEIPT_HASH_INVALID")
    ...
    if expected_stage == "E3_representative8":
        if (
            ...
            issues.add("RC0028_E3_PREMATURE_AUTHORIZATION_FORBIDDEN")
    elif (
        ...
        issues.add("RC0028_E3_PRODUCT_READ_AUTHORIZATION_REJECTED")
    return tuple(sorted(issues))


def validate_rc0028_e4_viability(value: Any) -> tuple[str, ...]:
    """Lock exact 100-case accounting and the external E3 authorization."""

    receipt_issues = validate_rc0028_experiment_body_free_receipt(
        value,
...
def _validate_body_free_receipt(value: Any) -> None:
    ...
    if type(value) is not dict or set(value) != expected_top:
        _stop("RC0028_BODY_FREE_RECEIPT_INVALID")
    cases = value.get("cases")
    counts = value.get("disposition_counts")
    source_hashes = value.get("source_fixture_hashes")
    if (
...
def capture_rc0027_baseline(output_directory: Path) -> None:
    ...
    try:
        ...
        if (
            ...
            _stop("RC0028_FROZEN_SOURCE_CHANGED_DURING_CAPTURE")
        ...
        if disposition_counts != _EXPECTED_DISPOSITION_COUNTS:
            _stop("RC0028_RC0027_BASELINE_AGGREGATE_MISMATCH")
        private_artifact = {
...
def _assert_rc0028_receipt_sources(
    ...
    *,
    ...
    issues = validate_rc0028_experiment_body_free_receipt(
        receipt,
        expected_stage=expected_stage,
    )
    if issues:
        _stop(
            "RC0028_E3_BODY_FREE_RECEIPT_INVALID"
            if expected_stage == "E3_representative8"
            else "RC0028_E4_BODY_FREE_RECEIPT_INVALID"
        )
    if (
    ...
    *,
    ...
    try:
        ...
        if evaluation_stage == "E3_representative8":
            ...
            _stop("RC0028_CLI_COMMAND_REJECTED")
        if len(selected_samples) != expected_count:
            _stop("RC0028_RC0028_CASE_SET_INVALID")

        private_rows: list[dict[str, Any]] = []
        body_free_rows: list[dict[str, Any]] = []
        for sample in selected_samples:
            case_id = sample["case_id"]
            private_row, body_free_row = _run_rc0028_case_projection(
                sample,
                source_case_commitment=commitments[case_id],
                source_closure=source_closure,
            )
        ...
        if (
            ...
            _stop("RC0028_FROZEN_SOURCE_CHANGED_DURING_RUN")
        if evaluation_stage == "E4_frozen100":
            ...
            if (
                ...
                _stop("RC0028_E3_PRODUCT_READ_AUTHORIZATION_CHANGED")
        ...
        if sum(disposition_counts.values()) != expected_count:
            _stop(
                "RC0028_E3_ACCOUNTING_MISMATCH"
                if evaluation_stage == "E3_representative8"
                else "RC0028_E4_ACCOUNTING_MISMATCH"
            )
        body_free_receipt = {
            "body_free": True,
        ...
        _assert_rc0028_receipt_sources(
            body_free_receipt,
...
def run_rc0028_representative8(output_directory: Path) -> None:
    """Run exactly the frozen E3 eight and emit private Product Read inputs."""

    _run_rc0028_exact_scope(
        output_directory,
    ...
    try:
        ...
        with redirect_stdout(io.StringIO()), redirect_stderr(io.StringIO()):
            if args.command == "capture-rc0027-baseline":
                ...
                success_code = b"RC0028_RC0027_BASELINE_CAPTURED\n"
            elif args.command == "run-representative8":
                run_rc0028_representative8(args.output_dir)
                success_code = b"RC0028_E3_REPRESENTATIVE8_COMPLETED\n"
            elif args.command == "run-frozen100":
                run_rc0028_frozen100(
                    args.output_dir,
                    product_read_receipt_path=(
                        args.product_read_receipt
                    ),
                )
                success_code = b"RC0028_E4_FROZEN100_COMPLETED\n"
            else:
                _stop("RC0028_CLI_COMMAND_REJECTED")
        ...
        os.write(
            2,
            b"RC0028_BOUNDED_EXPERIMENT_STOP:RC0028_CLI_ARGUMENT_REJECTED\n",
        )
        return 2
    except BoundedExperimentStop as exc:
        os.write(2, f"RC0028_BOUNDED_EXPERIMENT_STOP:{exc.code}\n".encode("ascii"))
        return 2
    except BaseException:
        os.write(2, b"RC0028_BOUNDED_EXPERIMENT_STOP:RC0028_UNHANDLED_EXCEPTION\n")
        return 2
    if not success_code:
        os.write(
            2,
            b"RC0028_BOUNDED_EXPERIMENT_STOP:RC0028_CLI_COMMAND_REJECTED\n",
        )
        return 2

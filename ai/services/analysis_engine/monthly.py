...
def assemble_monthly(weeks: List[WeeklySnapshot], period: str) -> MonthlyReport:
    ...
    return MonthlyReport(
        ...
        alternation_trend=alternation_trend,
        intensity_std_trend=intensity_std_trend,
        motif_trend=motif_trend,
...
def narrate_monthly(report: MonthlyReport) -> Narrative:
    ...
    if pivot is not None:
        parts.append(
            f"第{pivot + 1}週を境に、重心が{_label_ja(_top_label(weeks[pivot - 1].share))}から{_label_ja(_top_label(weeks[pivot].share))}方向へと移行した兆しがある。"
        )

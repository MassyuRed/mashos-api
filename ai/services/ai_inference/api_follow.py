...
LEGACY_FRIEND_REQUESTS_TABLE = "friend_requests"

# Current bridge view is backend read-only. Keep legacy write paths on
...
EMOTION_LOG_FEED_READ_TABLE = (
    os.getenv("COCOLON_EMOTION_LOG_FEED_READ_TABLE")
    or os.getenv("EMOTION_LOG_FEED_READ_TABLE")
    or os.getenv("COCOLON_FRIEND_EMOTION_FEED_READ_TABLE")
    or os.getenv("FRIEND_EMOTION_FEED_READ_TABLE")
    or "emotion_log_feed"
...
class FriendRequestCreateBody(BaseModel):
    share_code: Optional[str] = Field(
        ...
        min_length=4,
        max_length=64,
        description="Target user's current public share_code",
    )
    friend_code: Optional[str] = Field(
        default=None,
        min_length=4,
        max_length=64,
        description="Target user's legacy friend_code (compat fallback)",
    )
...
class FriendRemoveBody(BaseModel):
    target_user_id: Optional[str] = Field(
        ...
        min_length=1,
        max_length=128,
        description="Target user's current user id",
    )
    friend_user_id: Optional[str] = Field(
        default=None,
        min_length=1,
        max_length=128,
        description="Target user's legacy user id key (compat fallback)",
    )
...
class FriendNotificationSettingItem(BaseModel):
    owner_user_id: str = Field(..., description="Notification owner user id")
...
async def _fetch_current_incoming_pending_count(user_id: str) -> Optional[int]:
    try:
        return int(
            await sb_count(
        ...
        logger.warning("Supabase %s incoming pending count failed: %s", FOLLOW_REQUESTS_TABLE, exc)
        return None


async def _fetch_legacy_incoming_pending_count(user_id: str) -> Optional[int]:
    try:
        return int(
            await sb_count(
                f"/rest/v1/{LEGACY_FRIEND_REQUESTS_TABLE}",
                params={
        ...
        logger.warning("Supabase %s incoming pending count failed: %s", LEGACY_FRIEND_REQUESTS_TABLE, exc)
        return None


async def _fetch_incoming_pending_count(user_id: str) -> int:
    current = await _fetch_current_incoming_pending_count(user_id)
    if current is not None:
        return max(0, int(current))

    legacy = await _fetch_legacy_incoming_pending_count(user_id)
...
async def _fetch_latest_pending_request_head_and_count(user_id: str) -> Tuple[Optional[str], int]:
    try:
        ...
        logger.warning("Supabase %s head+count failed: %s", FOLLOW_REQUESTS_TABLE, exc)
        resp = None
    ...
    if resp is not None and resp.status_code < 300:
        ...
        if total_count is None:
            total_count = await _fetch_current_incoming_pending_count(user_id)
        return latest_created_at, max(0, int(total_count or 0))

    if resp is not None and resp.status_code >= 300:
        logger.warning("Supabase %s head+count failed: %s %s", FOLLOW_REQUESTS_TABLE, resp.status_code, resp.text[:1500])

    try:
        legacy_resp = await _sb_get(
            f"/rest/v1/{LEGACY_FRIEND_REQUESTS_TABLE}",
            params={
                "select": "id,created_at",
                "requested_user_id": f"eq.{user_id}",
        ...
        logger.warning("Supabase %s head+count failed: %s", LEGACY_FRIEND_REQUESTS_TABLE, exc)
    ...
    if legacy_resp.status_code >= 300:
        logger.warning("Supabase %s head+count failed: %s %s", LEGACY_FRIEND_REQUESTS_TABLE, legacy_resp.status_code, legacy_resp.text[:1500])
        latest_created_at = await _latest_friend_request_created_at(user_id)
        total_count = await _fetch_incoming_pending_count(user_id)
        return latest_created_at, total_count

    rows = legacy_resp.json()
    rows = rows if isinstance(rows, list) else []
    latest_created_at: Optional[str] = None
    if rows and isinstance(rows[0], dict):
        value = rows[0].get("created_at")
    ...
    total_count = _parse_content_range_total(legacy_resp.headers.get("content-range") or legacy_resp.headers.get("Content-Range") or "")
    if total_count is None:
        total_count = await _fetch_incoming_pending_count(user_id)
    return latest_created_at, max(0, int(total_count or 0))
...
async def get_friend_unread_status_payload_for_user(user_id: str) -> Dict[str, Any]:
    ...
    async def _build_payload() -> Dict[str, Any]:
        feed_last_read_at, requests_last_read_at, latest_feed_created_at, pending_request_head = await asyncio.gather(
            ...
            _emotion_log_feed_head_created_at_for_viewer(uid),
...
async def _emotion_log_feed_head_created_at_for_viewer(user_id: str) -> Optional[str]:
    """Return the canonical feed freshness head for the current viewer.

    Canonical EmotionLog read for current clients is artifact-first. We therefore
    prefer the latest READY summary head and only fall back to the live table
    when no READY artifact is available yet.
    """
    try:
        summary = await fetch_latest_ready_emotion_log_feed_summary(user_id)
    except Exception as exc:
        logger.warning("Failed to fetch ready emotion log feed summary head: %s", exc)
        summary = None

    summary_latest_created_at = _summary_latest_emotion_log_feed_created_at(summary)
    if summary_latest_created_at:
        return summary_latest_created_at
    return await _latest_emotion_log_feed_created_at(user_id)


async def _fetch_live_emotion_log_feed_rows(viewer_user_id: str, *, limit: int = 20) -> List[Dict[str, Any]]:
    resp = await _sb_get(
        f"/rest/v1/{EMOTION_LOG_FEED_READ_TABLE}",
        params={
...
async def _fetch_emotion_notification_map(
    ...
    try:
        ...
        if ids_filter is not None:
            logger.warning("Failed to query filtered %s, retrying unfiltered: %s", EMOTION_NOTIFICATION_SETTINGS_READ_TABLE, exc)
            return await _fetch_friend_notification_map(viewer_user_id, owner_user_ids=None)
        logger.warning("Failed to query %s: %s", EMOTION_NOTIFICATION_SETTINGS_READ_TABLE, exc)
        return {}
    ...
    if resp.status_code >= 300:
        if ids_filter is not None:
            logger.warning(
                "Supabase select filtered %s failed, retrying unfiltered: %s %s",
                EMOTION_NOTIFICATION_SETTINGS_READ_TABLE,
                resp.status_code,
                resp.text[:1500],
            )
...
async def _latest_friend_request_created_at(user_id: str) -> Optional[str]:
    try:
        ...
        logger.warning("Supabase %s latest failed: %s", FOLLOW_REQUESTS_TABLE, exc)
        resp = None
    ...
    if resp is not None and resp.status_code >= 300:
        logger.warning("Supabase %s latest failed: %s %s", FOLLOW_REQUESTS_TABLE, resp.status_code, resp.text[:1500])
    ...
    try:
        ...
        logger.warning("Supabase %s latest failed: %s", LEGACY_FRIEND_REQUESTS_TABLE, exc)
        return None

    if legacy_resp.status_code >= 300:
        logger.warning("Supabase %s latest failed: %s %s", LEGACY_FRIEND_REQUESTS_TABLE, legacy_resp.status_code, legacy_resp.text[:1500])
        return None
    rows = legacy_resp.json()
    if isinstance(rows, list) and rows and isinstance(rows[0], dict):
        value = rows[0].get("created_at")
...
# Current-vocabulary aliases. Physical DB names stay friend-compatible until DB rename.
FollowRequestCreateBody = FriendRequestCreateBody
FollowRequestCreateResponse = FriendRequestCreateResponse
FollowRequestActionResponse = FriendRequestActionResponse
FollowRemoveBody = FriendRemoveBody
FollowRemoveResponse = FriendRemoveResponse
FollowRequestsReadResponse = FriendUnreadMarkReadResponse


def register_follow_routes(app: FastAPI) -> None:
    """Register current Follow / request-management routes on the given FastAPI app."""

    @app.post("/follow/request", response_model=FriendRequestCreateResponse)
    async def create_friend_request(
        ...
        share_code = str(body.share_code or body.friend_code or "").strip()
        if not share_code:
            raise HTTPException(status_code=400, detail="share_code is required")

        target_profile = await _lookup_profile_by_friend_code(share_code)
        if not target_profile:
            raise HTTPException(status_code=404, detail="User not found for the given share_code")
        ...
        if await _are_already_friends(requester_user_id, requested_user_id):
            return FriendRequestCreateResponse(
                ...
                target_display_name=requested_display_name,
    ...
    @app.post("/follow/requests/{request_id}/accept", response_model=FriendRequestActionResponse)
    ...
    @app.post("/follow/requests/{request_id}/reject", response_model=FriendRequestActionResponse)
    ...
    @app.post("/follow/requests/{request_id}/cancel", response_model=FriendRequestActionResponse)
    ...
    @app.post("/follow/remove", response_model=FriendRemoveResponse)
    async def remove_friend(
        ...
        me = await _resolve_user_id_from_token(access_token)
        target_user_id = (body.target_user_id or body.friend_user_id or "").strip()
        if not target_user_id:
            raise HTTPException(status_code=400, detail="target_user_id is required")
        if target_user_id == me:
            raise HTTPException(status_code=400, detail="You cannot remove yourself from friends")

        await _delete_friendships_bidirectional(me, target_user_id)
        await _invalidate_friend_api_caches(me, target_user_id)
    ...
    @app.get("/_compat-retired/friends/manage")
    ...
    @app.post("/follow/requests/read", response_model=FriendUnreadMarkReadResponse)
...
def register_emotion_log_routes(app: FastAPI) -> None:
    """Delegate EmotionLog routes to the current EmotionLog owner."""
    from api_emotion_log import register_emotion_log_routes as _register_emotion_log_routes

    _register_emotion_log_routes(app)


def register_emotion_notification_routes(app: FastAPI) -> None:
    """Delegate EmotionNotification routes to the current settings owner."""
    from api_emotion_notification_settings import (
        register_emotion_notification_settings_routes as _register_emotion_notification_settings_routes,
    )

    _register_emotion_notification_settings_routes(app)


def register_friend_routes(app: FastAPI) -> None:
    """Legacy aggregate registrar kept for backwards-compatible imports."""

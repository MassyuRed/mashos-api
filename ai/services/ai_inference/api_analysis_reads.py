...
# Current-vocabulary aliases. DB/table names stay MyWeb-compatible until the DB rename phase.
AnalysisWeeklySummary = MyWebWeeklySummary
AnalysisMonthlySummary = MyWebMonthlySummary
AnalysisInputStatusSummary = MyWebInputStatusSummary
AnalysisHomeSummaryResponse = MyWebHomeSummaryResponse
AnalysisWeeklyDay = MyWebWeeklyDay
AnalysisWeeklyDaysResponse = MyWebWeeklyDaysResponse
get_analysis_home_summary_payload_for_user = get_myweb_home_summary_payload_for_user
_fetch_analysis_report_row = _fetch_myweb_report_row


def register_analysis_read_routes(app: FastAPI) -> None:
    """Register current Analysis read routes on the given FastAPI app."""

    @app.get("/analysis/home-summary", response_model=AnalysisHomeSummaryResponse)
    async def get_analysis_home_summary(
        ...
        return AnalysisHomeSummaryResponse(**payload)

    @app.get("/analysis/reports/{report_id}/weekly-days", response_model=AnalysisWeeklyDaysResponse)
    async def get_analysis_report_weekly_days(
        ...
        if saved_days:
            return AnalysisWeeklyDaysResponse(

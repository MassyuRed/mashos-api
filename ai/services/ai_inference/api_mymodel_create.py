...
def _build_create_status_entry(question_rows: List[Dict[str, Any]], answers: Dict[int, Dict[str, Any]], *, build_tier: str) -> Dict[str, Any]:
    question_ids: List[int] = []
    for row in question_rows or []:
        try:
            question_ids.append(int(row.get("id")))
        except Exception:
            continue
    ...
    for qid in question_ids:
        ans = answers.get(int(qid)) if isinstance(answers, dict) else None
        if isinstance(ans, dict) and str(ans.get("answer_text") or "").strip():
            answered_count += 1

    unanswered_count = max(0, int(total_questions) - int(answered_count))
    has_unanswered = bool(total_questions > 0 and unanswered_count > 0)
    return {
        "build_tier": str(build_tier or "light"),
        "total_questions": int(total_questions),
        "answered_count": int(answered_count),
        "unanswered_count": int(unanswered_count),
        "has_unanswered": bool(has_unanswered),
        "is_created": bool(answered_count > 0),
    }


async def get_mymodel_create_status_payload_for_user(user_id: str) -> Dict[str, Any]:
    uid = str(user_id or "").strip()
    default_entry = _build_create_status_entry([], {}, build_tier="light")
    default_payload = {
        "status": "ok",
        "user_id": uid,
        "subscription_tier": SubscriptionTier.FREE.value,
        "effective_build_tier": "light",
        "light": dict(default_entry),
        "standard": dict(default_entry),
        "accessible": dict(default_entry),
        "has_unanswered": False,
        "has_any_unanswered": False,
        "is_created": False,
        "engine": "mymodel.create.status.v1",
    }
    default_payload["standard"]["build_tier"] = "standard"
    if not uid:
        return default_payload

    try:
        tier_enum = await get_subscription_tier_for_user(uid)
    except Exception:
        tier_enum = SubscriptionTier.FREE

    entitlement = resolve_mymodel_entitlement(tier_enum)
    effective_build_tier = str(getattr(entitlement, "build_tier", "light") or "light")

    all_active_questions = await _fetch_questions_all_active()
    light_questions = filter_question_rows_for_build_tier(all_active_questions, build_tier="light")
    standard_questions_all = filter_question_rows_for_build_tier(all_active_questions, build_tier="standard")
    standard_questions = light_questions if effective_build_tier == "light" else standard_questions_all

    question_ids: Set[int] = set()
    for row in (light_questions or []):
        try:
            question_ids.add(int(row.get("id")))
        except Exception:
            continue
    for row in (standard_questions or []):
        try:
            question_ids.add(int(row.get("id")))
        except Exception:
            continue

    answers = await _fetch_answers(user_id=uid, question_ids=question_ids) if question_ids else {}

    light_status = _build_create_status_entry(light_questions, answers, build_tier="light")
    standard_status = _build_create_status_entry(standard_questions, answers, build_tier="standard")
    accessible_status = dict(light_status if effective_build_tier == "light" else standard_status)

    return {
        "status": "ok",
        "user_id": uid,
        "subscription_tier": getattr(tier_enum, "value", str(tier_enum or SubscriptionTier.FREE.value)),
        "effective_build_tier": effective_build_tier,
        "light": light_status,
        "standard": standard_status,
        "accessible": accessible_status,
        "has_unanswered": bool(accessible_status.get("has_unanswered")),
        "has_any_unanswered": bool(light_status.get("has_unanswered") or standard_status.get("has_unanswered")),
        "is_created": bool(accessible_status.get("is_created")),
        "engine": "mymodel.create.status.v1",
    }

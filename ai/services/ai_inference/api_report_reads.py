...
from publish_governance import decide_myprofile_report_publish, decide_myweb_report_publish
...
async def _fetch_latest_self_structure_ids(user_id: str, *, limit: int) -> List[str]:
    resp = await sb_get(
        "/rest/v1/myprofile_reports",
        params={
            "select": "id,report_type,content_text,content_json",
            "user_id": f"eq.{user_id}",
    ...
    ids: List[str] = []
    for row in _pick_rows(resp):
        published_row = decide_myprofile_report_publish(row, requested_report_type="monthly")
        if not published_row:
            continue
        rid = str(published_row.get("id") or "").strip()
        if rid:
...
def register_report_reads_routes(app: FastAPI) -> None:
    ...
    async def get_myweb_unread_status(
        ...
        ids_by_type = {
            "daily": await _fetch_latest_ready_myweb_ids(me, "daily", tier_str=tier_str, limit=lim),
            "weekly": await _fetch_latest_ready_myweb_ids(me, "weekly", tier_str=tier_str, limit=lim),
            "monthly": await _fetch_latest_ready_myweb_ids(me, "monthly", tier_str=tier_str, limit=lim),
            "selfStructure": [],
        }

        if include_self_structure and tier_str in {"plus", "premium"}:
            try:
                ids_by_type["selfStructure"] = await _fetch_latest_self_structure_ids(me, limit=lim)
            except HTTPException:
                raise
            except Exception as exc:
                logger.warning("failed to load self-structure unread ids: %s", exc)
                ids_by_type["selfStructure"] = []

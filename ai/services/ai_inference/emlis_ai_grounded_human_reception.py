...
_SOURCE_GROUNDED_RELATION_FRAMES: Final[
    ...
    "attempt_and_block": _SourceGroundedRelationFrameV1(
        ("LEFT", "RIGHT"), ("と", "が"), (None, None),
        "せめぎ合い", "せめぎ合っている", "synthesize_relation",
    ),
    ...
    "evaluation_about_event": _SourceGroundedRelationFrameV1(
        ("LEFT", "RIGHT"), ("が", "を"), ("FROM", "TO"),
        "限定し", "限定している", "synthesize_relation",
    ),
    ...
    *,
    ...
    def relation_nominal_for_slot(
        ...
        if (
            ...
            or relation_occurrences_by_slot[semantic_slot] > 1
        ):
            return typed_nominal_by_slot[semantic_slot]
        return _source_grounded_anaphoric_nominal(
            move.semantic_heads[semantic_slot],
            predicate_kind=move.predicate_kind,
        )
...
def _source_grounded_meaning_fragment(
    ...
    *,
    ...
    return (
        _SOURCE_GROUNDED_PREDICATE_ATTRIBUTIVE[move.predicate_kind]
        if move.reference_mode == "ANAPHORIC"
        else move.semantic_heads[semantic_slot]
    )
    ...
    *,
    ...
    expected_time, expected_aspect, expected_time_adjunct, expected_aspect_adjunct = (
        _source_grounded_temporal_aspect_realization(
            realization,
            realization.semantic_heads[target_owner_slot],
        )
    )
    if (
    ...
    *,
    ...
    try:
        (
            ...
            # Reception is a non-terminal operator over the target core.
            # The governed terminal lemma comes from the target predicate,
            # so distinct semantic clause frames cannot collapse into one
            # completed-string close.
            "present_direction": (
                "尊重したうえで、",
                "先回りせずに",
                "見守る",
    ...
    try:
        voice_complement = {
            "SELF_PERFORMED": "本人から始まったことを保ち、",
            "OTHER_PERFORMED": "本人以外の働きを取り違えず、",
            "FUTURE_INTENTION": "まだ実行済みとはせず、",
            "RECEIVED": "受け取った側の経験として、",
            "STATE": "",
        }[voice]
    except KeyError:
        raise GroundedHumanReceptionSurfaceError(
    ...
    if reception_lemma == predicate_lemma:
        raise GroundedHumanReceptionSurfaceError(
            "MEANING_REALIZATION_CAPABILITY_GAP"
        )

    if future_action:
        act_guard = "これからの行動を軽く扱わず、"
    else:
        try:
            act_guard = {
                "stay_with_current_burden": "軽く扱わず、",
                "honor_concrete_effort": "軽いこととして流さず、",
                "protect_retained_intention": "心から消さず、",
                "recognize_lived_change": "見過ごさず、",
                "hold_help_seeking": "",
...
def bind_and_validate_grounded_human_reception_surface(
    ...
    *,
    ...
    semantic_profile_by_move: dict[str, _ReceptionSemanticProfileV1] = {}
    anchor_used = False
    for clause_plan in resolved_clause_plans:
        for move_id in clause_plan.move_ids:
            ...
            semantic_profiles, target_slot_count = (
                _source_grounded_move_voice_profiles(
                    move,
                    nucleus_index,
                    resolver,
                )
            )
            target_owner_slot = _source_grounded_target_owner_from_profiles(
                semantic_profiles,
                target_slot_count,
                referent.kind,
            )
            voice_by_move[move_id] = _source_grounded_profile_voice(
                semantic_profiles[target_owner_slot],
                referent.kind,
            )
            semantic_profile_by_move[move_id] = semantic_profiles[
                target_owner_slot
            ]
    ...
    for clause_plan, clause_text in zip(
        ...
        for move in clause_moves:
            ...
            visible_referent = (
                referent.text.replace("「", "").replace("」", "")
                if referent is not None
                else ""
            )
            if visible_referent:
                moves_by_referent_text.setdefault(
                    visible_referent,
                    [],
                ).append(move)
        for visible_referent, moves in moves_by_referent_text.items():
            occurrence_count = _visible_fragment_occurrence_count(
                clause_text,
                (visible_referent,),
            )
            if occurrence_count < len(moves):
                issues.extend(
                    f"human_reception_move_target_missing:{move.move_id}"
                    for move in moves
                )
            elif occurrence_count > len(moves):
                issues.extend(
                    f"human_reception_move_target_duplicate:{move.move_id}"
                    for move in moves
                )

        referent_starts: list[tuple[GroundedReceptionMovePlan, int]] = []
        search_cursor = 0
        for move in clause_moves:
            referent = referent_by_move.get(move.move_id)
            visible_referent = (
            ...
            referent_start = (
                clause_text.find(visible_referent, search_cursor)
                if visible_referent
                else -1
            )
            if referent_start < 0:
                ...
                continue
            referent_starts.append((move, referent_start))
            search_cursor = referent_start + len(visible_referent)
        if len(referent_starts) == len(clause_moves):
            # A witness is owned only when its act predicate begins after this
            # Move's unique referent and before the next Move's referent.  Its
            # full interval may extend beyond that loose boundary; requiring
            # all selected intervals to be disjoint then closes cross-Move
            # predicate borrowing without imposing a lexical length window on
            # a relation core.
            witnesses: list[
                tuple[GroundedReceptionMovePlan, int, int] | None
            ] = []
            for index, (move, start) in enumerate(referent_starts):
                next_start = (
                    referent_starts[index + 1][1]
                    if index + 1 < len(referent_starts)
                    else len(clause_text)
                )
                owned_end = next_start
                if index + 1 < len(referent_starts):
                    next_move = referent_starts[index + 1][0]
                    next_context_ids = _dedupe(
                        context_map.get(next_move.move_id, ())
                    )
                    next_reference = reception_effective_move_reference_mode(
                        reception_plan,
                        next_move,
                        recovery_stage,
                    )
                    context_start = next_start
                    if next_context_ids and next_reference == "anaphoric_first":
                        marker_starts = tuple(
                            match.start()
                            for match in _ANAPHORIC_CONTEXT_MARKER_RE.finditer(
                                clause_text,
                                start,
                                next_start,
                            )
                    ...
                    if join_boundary <= start:
                        issues.append(
                            "human_reception_move_owned_slice_missing:"
                            f"{move.move_id}"
                        )
                        witnesses.append(None)
                        continue
                    owned_end = join_boundary
                referent = referent_by_move.get(move.move_id)
                future_action = bool(
                    move.reception_act == "honor_concrete_effort"
                    and referent is not None
                    and referent.kind == "future_action_intention"
                )
                if move.move_role == "bounded_counterposition":
                    candidates = tuple(
                        (start, match.end())
                        for match in _ACT_OWNED_RESPONSIBILITY_RE[
                            move.reception_act
                        ].finditer(clause_text, start, owned_end)
                    )
                else:
                    expected_predicate = (
                        _source_grounded_response_predicate_surface(
                            move.reception_act,
                            move.move_role,
                            future_action=future_action,
                            target_predicate_kind=(
                                _source_grounded_referent_predicate_kind(
                                    referent.kind
                                )
                            ),
                            semantic_profile=semantic_profile_by_move[
                                move.move_id
                            ],
                            referent_kind=referent.kind,
                            voice=voice_by_move[move.move_id],
                            clause_form=(
                                "FINITE"
                                if index + 1 == len(referent_starts)
                                else "CONTINUATIVE"
                ...
                if len(candidates) != 1:
                    ...
                    if move.move_role == "attention" and candidates:
                        issues.append(
                            "human_reception_move_attention_missing:"
                            f"{move.move_id}"
                        )
                    continue
                witness_start, witness_end = candidates[0]
                witnesses.append((move, witness_start, witness_end))

            owned_intervals = tuple(
                (start, end)
                for witness in witnesses
                if witness is not None
                for _move, start, end in (witness,)
            )
            if any(
                left_start < right_end and right_start < left_end
                for interval_index, (left_start, left_end) in enumerate(
                    owned_intervals
                )
                for right_start, right_end in owned_intervals[
                    interval_index + 1 :
                ]
            ):
                for witness in witnesses:
    ...
    for move in active_moves:
        ...
        responsibility_text = owned_text_by_move.get(move.move_id, "")
        referent = referent_by_move.get(move.move_id)
        if not _ACT_OWNED_RESPONSIBILITY_RE[
            move.reception_act
        ].search(responsibility_text):
            issues.append(
                "human_reception_act_responsibility_missing:"
                f"{move.reception_act}"
            )
        if (
            ...
            and not _ATTENTION_RESPONSIBILITY_RE.search(responsibility_text)
        ):
            issues.append(
        ...
        referent_visible = bool(
            ...
            and (
                referent.text in move_text
                or referent.text.replace("「", "").replace("」", "")
                in move_text
            )
        )
        ...
        def _is_integrated_target_head(fragment: str) -> bool:
            return bool(
                ...
                and _visible_fragment_occurrence_count(
                    move_text,
                    (fragment,),
                )
                == 1
        ...
        if effective_reference == "short_anchor_if_ambiguous" and any(
            ...
            and not _is_integrated_target_head(fragment)

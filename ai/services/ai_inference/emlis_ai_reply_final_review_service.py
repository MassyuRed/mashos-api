...
BROKEN_NOUN_PHRASE_RE = re.compile(r"(?:(だ|だから|けど|けれど|から)(気持ち|思い|願い|状態)|(?:離れ|分かれ|外れ|崩れ|揺れ|逃げ|戻れ|整え|貯め|決め|始め|頼り|頑張り|楽しみ|守り|選び|進み)こと)")
...
def review_emlis_ai_reply_text(*, comment_text: Any, world_model: Optional[WorldModel] = None) -> FinalReviewResult:
    ...
    tone_engine_2_1_report = build_runtime_surface_tone_engine_2_1_report(comment_text=final_text)
    tone_engine_2_1_block_remaining = bool(tone_engine_2_1_report.get("release_blocker"))
    ...
    passed = bool(final_text) and not block_remaining and not repetition_remaining and not tone_engine_2_1_block_remaining and not underanswered and not presence_missing
    if stale_remaining:
    ...
    if tone_engine_2_1_block_remaining:
        for reason in list(tone_engine_2_1_report.get("blocker_reasons") or tone_engine_2_1_report.get("tone_guard_reasons") or []):
            issues.append(FinalReviewIssue(code=f"tone_engine_2_1_{reason}", severity="block", line_index=None, message=str(reason)))
    return FinalReviewResult(passed=passed, issues=issues, repaired_text=repaired_text, review_version=f"emlis.final_reader.v1+{RUNTIME_SURFACE_TONE_ENGINE_2_1_VERSION}")

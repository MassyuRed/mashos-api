...
- "body"  := profile_create_answers.reflection_display_text (current bridge view; fallback: formatted answer_text)
...
from piece_generated_access import resolve_generated_reflection_access
from piece_text_formatter import get_public_create_reflection_text
...
# Tables (overrideable)
# Current bridge views are backend read-only. Keep write/upsert/delete paths on legacy
# physical tables, and use dedicated READ_TABLE constants only for SELECT-only paths.
...
# ProfileCreate answer reads are SELECT-only and can use the current-name bridge view.
# ProfileCreate writes/deletes stay on public.mymodel_create_answers in api_profile_create.py.
...
async def _fetch_holder_user_ids_for_question(*, question_id: int, scan_limit: int) -> List[str]:
    ...
    qid = int(question_id)
    resp = await _sb_get(
        f"/rest/v1/{PROFILE_CREATE_ANSWERS_READ_TABLE}",
        params={
            "select": "*",
    ...
    if resp.status_code >= 300:
        logger.error("Supabase %s select failed: %s %s", PROFILE_CREATE_ANSWERS_READ_TABLE, resp.status_code, resp.text[:1500])
...
async def _fetch_active_user_ids(*, since_iso: str, scan_limit: int) -> Set[str]:
    ...
    - OR profile_create_answers.updated_at >= since
    ...
    # 2) profile_create_answers (current-name read bridge)
    try:
        resp2 = await _sb_get(
            f"/rest/v1/{PROFILE_CREATE_ANSWERS_READ_TABLE}",
            params={
        ...
        if resp2.status_code < 300:
            ...
            logger.error("Supabase %s select failed: %s %s", PROFILE_CREATE_ANSWERS_READ_TABLE, resp2.status_code, resp2.text[:800])
    except Exception as exc:
        logger.warning("active users: %s fetch failed: %s", PROFILE_CREATE_ANSWERS_READ_TABLE, exc)
...
async def _fetch_metrics(q_keys: Set[str]) -> Dict[str, Dict[str, Any]]:
    ...
    if resp.status_code >= 300:
        logger.error("Supabase %s select failed: %s %s", METRICS_READ_TABLE, resp.status_code, resp.text[:1500])
...
async def _fetch_reads(viewer_user_id: str, q_instance_ids: Set[str]) -> Set[str]:
    ...
    if resp.status_code >= 300:
        logger.error("Supabase %s select failed: %s %s", READS_READ_TABLE, resp.status_code, resp.text[:1500])
        return set()
    rows = resp.json()
    out: Set[str] = set()
    if isinstance(rows, list):
...
# ---------------------------------------------------------
    ...
    @app.get("/_compat-retired/mymodel/qna/discoveries/reflections", response_model=QnaSavedReflectionsResponse)
    ...
    @app.post("/_compat-retired/mymodel/qna/discoveries/submit", response_model=QnaDiscoverySubmitResponse)
    ...
    @app.post("/_compat-retired/mymodel/qna/discoveries/delete", response_model=QnaDiscoveryDeleteResponse)
    ...
    @app.get("/_compat-retired/mymodel/qna/discoveries/history", response_model=QnaDiscoveryHistoryResponse)
...
async def _fetch_generated_public_group_rows_for_owner(
    ...
    *,
    ...
    for params in query_sets:
        rows = await _sb_get_json_local(f"/rest/v1/{MYMODEL_REFLECTIONS_READ_TABLE}", params=params)
        for row in rows:
...
# Current-vocabulary aliases. Physical DB names stay MyModel/QnA-compatible until DB rename.
PieceLibraryResponse = QnaListResponse
PieceDetailResponse = QnaDetailResponse
PieceViewResponse = QnaViewResponse
PieceResonanceResponse = QnaResonanceResponse
PieceResonanceSubmitResponse = QnaEchoesSubmitResponse
PieceResonanceDeleteResponse = QnaEchoesDeleteResponse
PieceResonanceHistoryResponse = QnaEchoesHistoryResponse
PieceResonancePiecesResponse = QnaSavedReflectionsResponse
PieceUnreadResponse = QnaUnreadResponse
PieceUnreadStatusResponse = QnaUnreadStatusResponse
PieceRecommendUsersResponse = QnaRecommendUsersResponse
PieceViewRequest = QnaViewRequest
PieceResonanceRequest = QnaResonanceRequest
PieceResonanceSubmitRequest = QnaEchoesSubmitRequest
PieceResonanceDeleteRequest = QnaEchoesDeleteRequest


def register_piece_runtime_routes(app: FastAPI) -> None:
    """Register current Piece runtime routes on the given FastAPI app."""
    @app.get("/piece/library", response_model=PieceLibraryResponse)
    async def qna_list(
        target_user_id: Optional[str] = Query(default=None, description="Owner of the MyModel (defaults to viewer)"),
        sort: str = Query(default="newest", description="newest | popular"),
        metric: str = Query(default="views", description="views | resonances (compatibility only)"),
    ...
    @app.get("/piece/unread", response_model=PieceUnreadResponse)
    async def qna_unread(
        target_user_id: Optional[str] = Query(default=None, description="Owner of the MyModel (defaults to viewer)"),
        authorization: Optional[str] = Header(default=None, alias="Authorization"),
    ) -> PieceUnreadResponse:
    ...
    @app.get("/piece/unread-status", response_model=PieceUnreadStatusResponse)
    ...
    @app.get("/piece/detail", response_model=PieceDetailResponse)
    async def qna_detail(
        q_instance_id: str = Query(..., description="reflection:<uuid>"),
        mark_viewed: bool = Query(default=False, description="If true, also mark read and apply view counts in the same request"),
        include_my_discovery_latest: bool = Query(default=False, description="Compatibility flag; discoveries are retired and ignored by the canonical public read path"),
    ...
    @app.post("/piece/view", response_model=PieceViewResponse)
    async def qna_view(
        req: PieceViewRequest,
        authorization: Optional[str] = Header(default=None, alias="Authorization"),
    ) -> PieceViewResponse:
        token = _extract_bearer_token(authorization) if authorization else None
        if not token:
    ...
    @app.post("/piece/resonance", response_model=PieceResonanceResponse)
    async def qna_resonance(
        req: PieceResonanceRequest,
        authorization: Optional[str] = Header(default=None, alias="Authorization"),
    ...
    async def qna_echoes_submit(
        req: PieceResonanceSubmitRequest,
        authorization: Optional[str] = Header(default=None, alias="Authorization"),
    ...
    @app.post("/piece/resonances/delete", response_model=PieceResonanceDeleteResponse)
    async def qna_echoes_delete(
        req: PieceResonanceDeleteRequest,
        authorization: Optional[str] = Header(default=None, alias="Authorization"),
    ...
    @app.get("/piece/resonances/history", response_model=PieceResonanceHistoryResponse)
    async def qna_echoes_history(

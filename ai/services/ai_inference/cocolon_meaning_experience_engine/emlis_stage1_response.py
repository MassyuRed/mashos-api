...
def _partition_shared_reception_move_contributions(rows, reception_plan, binding):
    ...
    from emlis_ai_grounded_observation_plan import _thread_retained_reaction_groups, _source_independent_positive_feelings, _source_explicit_original_feeling, _source_current_cognition
    ...
    grouped = bool(2 <= len(rows) <= 3
        and (withdrawal or any(move.support_nucleus_ids for move in reception_plan.moves)
             and ({"stay_with_current_burden", "recognize_lived_change"} <= {row.reception_act for row in rows}
                  or any(_source_explicit_original_feeling(n) or _source_current_cognition(n) for n in binding.node_meta.values()))
    ...
    if (material_group and any(m.target_nucleus_ids == (material_group[1].nucleus_id,)
        ...
        and _source_material_allows_reverse(material_group)):

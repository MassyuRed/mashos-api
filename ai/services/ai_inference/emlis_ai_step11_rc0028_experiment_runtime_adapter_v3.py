...
"""Disconnected runtime composition for the rc0028 Step 11 experiment.
...
STEP11_RC0028_EXPERIMENT_RUNTIME_RESULT_SCHEMA = (
    "cocolon.emlis.nls_v3.step11.rc0028_experiment_runtime_result.v1"
)
STEP11_RC0028_EXPERIMENT_PRIVATE_EXECUTION_SCHEMA = (
    "cocolon.emlis.nls_v3.step11.rc0028_experiment_private_execution.v1"
)
STEP11_RC0028_EXPERIMENT_RUNTIME_ADAPTER_VERSION = (
    "cocolon.emlis.nls_v3.runtime_adapter.step11.rc0028.experiment."
    "20260719.v1"
)
STEP11_RC0028_EXPERIMENT_EXECUTION_SCOPE = "offline_bounded_experiment"
STEP11_RC0028_BASE_CANDIDATE_VERSION_ID = "nls_v3_rc_0027"
STEP11_RC0028_EXPERIMENT_CANDIDATE_VERSION_ID = (
    "nls_v3_rc_0028_experiment"
)
STEP11_RC0028_MAX_CANDIDATES = 12
STEP11_RC0028_MAX_REPLANS = 1

_SHA256_RE = re.compile(r"^[0-9a-f]{64}$")
_CASE_ID_RE = re.compile(r"^[A-Za-z0-9][A-Za-z0-9_.:-]{0,95}$")
_CLOSED_CODE_RE = re.compile(r"^STEP11_RC0028_[A-Z0-9_]{2,79}$")
_SHAREABLE_FAILURE_CODE_RE = re.compile(
    r"^(?:STEP11_RC0028_[A-Z0-9_]{2,79}|S11_GATE[A-Z0-9_]{0,120})$"
)
_DISPOSITIONS = frozenset({"selected", "no_valid_candidate", "fail_close"})


class Step11Rc0028ExperimentRuntimeError(ValueError):
    """Caller-contract error containing one closed rc0028 code."""

    def __init__(self, code: str) -> None:
        if type(code) is not str or _CLOSED_CODE_RE.fullmatch(code) is None:
            code = "STEP11_RC0028_RUNTIME_ERROR_CODE_INVALID"
        self.code = code
        super().__init__(code)


@dataclass(frozen=True, slots=True)
class Step11Rc0028ExperimentBodyFreeResult:
...
Step11Rc0028ExperimentRuntimeResult = (
    Step11Rc0028ExperimentBodyFreeResult
)


@dataclass(frozen=True, slots=True, repr=False)
class Step11Rc0028ExperimentPrivateExecution:
    ...
    body_free_result: Step11Rc0028ExperimentBodyFreeResult
...
def _closed_pipeline_code(error: BaseException) -> str:
    ...
    if type(code) is str and code.startswith("STEP11_RUNTIME_"):
        return "STEP11_RC0028_BASE_RUNTIME_FAIL_CLOSE"
    return "STEP11_RC0028_PIPELINE_FAIL_CLOSE"


def _result_payload(
    value: Step11Rc0028ExperimentBodyFreeResult,
) -> dict[str, Any]:
    return {
...
def _make_result(
    *,
    ...
    provisional = Step11Rc0028ExperimentBodyFreeResult(
        schema_version=STEP11_RC0028_EXPERIMENT_RUNTIME_RESULT_SCHEMA,
        adapter_version=STEP11_RC0028_EXPERIMENT_RUNTIME_ADAPTER_VERSION,
        execution_scope=STEP11_RC0028_EXPERIMENT_EXECUTION_SCOPE,
        case_id=case_id,
        source_case_commitment=source_case_commitment,
        source_dependency_closure_sha256=(
            source_dependency_closure_sha256
        ),
        base_candidate_version_id=STEP11_RC0028_BASE_CANDIDATE_VERSION_ID,
        experiment_candidate_version_id=(
            STEP11_RC0028_EXPERIMENT_CANDIDATE_VERSION_ID
        ),
    ...
    result = Step11Rc0028ExperimentBodyFreeResult(
        **{
    ...
    issues = validate_step11_rc0028_experiment_result(result)
    if issues:
        raise Step11Rc0028ExperimentRuntimeError(issues[0])
    return result
...
def _gate_failure_counts(gate_results: tuple[Any, ...]) -> tuple[tuple[str, int], ...]:
    ...
    for gate in gate_results:
        ...
        if type(codes) not in {tuple, list}:
            raise Step11Rc0028ExperimentRuntimeError(
                "STEP11_RC0028_GATE_RESULT_INVALID"
            )
        for code in codes:
            if (
                ...
                raise Step11Rc0028ExperimentRuntimeError(
                    "STEP11_RC0028_GATE_FAILURE_CODE_INVALID"
                )
...
def _private_execution(
    *,
    ...
    selected_parsed_witness: Any | None,
    selected_verified_binding: Any | None,
) -> Step11Rc0028ExperimentPrivateExecution:
    return Step11Rc0028ExperimentPrivateExecution(
        schema_version=STEP11_RC0028_EXPERIMENT_PRIVATE_EXECUTION_SCHEMA,
        adapter_version=STEP11_RC0028_EXPERIMENT_RUNTIME_ADAPTER_VERSION,
...
def _execute_step11_rc0028_experiment_internal(
    ...
    *,
    ...
    if (
        ...
        or not 0 <= replan_limit <= STEP11_RC0028_MAX_REPLANS
    ):
        raise Step11Rc0028ExperimentRuntimeError(
    ...
    try:
        ...
        from emlis_ai_step11_natural_surface_v3 import (
            STEP11_RC0028_EXPERIMENT_CANDIDATE_VERSION_ID as surface_version,
        ...
        if surface_version != STEP11_RC0028_EXPERIMENT_CANDIDATE_VERSION_ID:
            raise Step11Rc0028ExperimentRuntimeError(
                "STEP11_RC0028_CANDIDATE_VERSION_MISMATCH"
            )
        baseline = execute_step11_offline_v3(
            dict(current_input),
            candidate_version_id=STEP11_RC0028_BASE_CANDIDATE_VERSION_ID,
            source_dependency_closure_sha256=(
                source_dependency_closure_sha256
            ),
        )
        if validate_step11_runtime_execution(baseline):
            raise Step11Rc0028ExperimentRuntimeError(
                "STEP11_RC0028_BASE_RUNTIME_VALIDATION_FAILED"
            )
        if baseline.status == "selected":
            ...
            raise Step11Rc0028ExperimentRuntimeError(
                "STEP11_RC0028_BASE_RUNTIME_STATUS_INVALID"
            )
        ...
        if not base_candidates:
            body_free_result = _make_result(
                ...
                closed_failure_codes=(
                    "STEP11_RC0028_BASE_CANDIDATE_SET_EMPTY",
                ),
        ...
        if validate_step11_rc0028_experiment_lexical_atom_specs(
            ...
            raise Step11Rc0028ExperimentRuntimeError(
                "STEP11_RC0028_LEXICAL_ATOM_SPECS_INVALID"
            )
        ...
        for candidate in experiment_candidates:
            if validate_step11_rc0028_experiment_surface_candidate(
                candidate,
                successor_snapshot=successor,
                lexical_atom_specs=lexical_specs,
            ):
            ...
            parsed = parse_step11_rc0028_experiment_surface(
                candidate.final_utf8_bytes
            )
            binding = match_step11_rc0028_experiment_surface(
                parsed,
                successor_snapshot=successor,
            )
        ...
        selection = select_step11_rc0028_experiment_candidate(
            experiment_candidates,
            **selector_kwargs,
        )
        repeated_selection = select_step11_rc0028_experiment_candidate(
            experiment_candidates,
            **selector_kwargs,
        )
        if selection != repeated_selection:
            raise Step11Rc0028ExperimentRuntimeError(
                "STEP11_RC0028_SELECTION_NONDETERMINISTIC"
            )
        ...
        if (
            ...
            raise Step11Rc0028ExperimentRuntimeError(
                "STEP11_RC0028_SELECTION_COMMITMENT_MISMATCH"
            )
        ...
        if selection.status == "selected":
            ...
            if (
                ...
                raise Step11Rc0028ExperimentRuntimeError(
                    "STEP11_RC0028_SELECTED_CANDIDATE_INVALID"
                )
        ...
        elif selection.status == "rc0028_experiment_no_valid_candidate":
            if (
                ...
                raise Step11Rc0028ExperimentRuntimeError(
                    "STEP11_RC0028_NO_VALID_SELECTION_INVALID"
                )
            disposition = "no_valid_candidate"
            closed_failure_codes = tuple(
                code for code, _count in gate_failure_code_counts
            ) or ("STEP11_RC0028_NO_VALID_CANDIDATE",)
            ...
            raise Step11Rc0028ExperimentRuntimeError(
                "STEP11_RC0028_SELECTION_STATUS_INVALID"
            )
...
def execute_step11_rc0028_experiment_private(
    current_input: Mapping[str, Any],
    *,
    case_id: str,
    source_case_commitment: str,
    ...
    return _execute_step11_rc0028_experiment_internal(
        current_input,
        case_id=case_id,
        source_case_commitment=source_case_commitment,
        source_dependency_closure_sha256=(
...
def run_step11_rc0028_experiment(
    current_input: Mapping[str, Any],
    *,
    case_id: str,
    source_case_commitment: str,
...
def validate_step11_rc0028_experiment_result(value: Any) -> tuple[str, ...]:
    """Validate the exact closed body-free receipt and its commitment."""

    if type(value) is not Step11Rc0028ExperimentBodyFreeResult:
        return ("STEP11_RC0028_RESULT_TYPE_INVALID",)
    issues: set[str] = set()
    if (
        ...
        != STEP11_RC0028_BASE_CANDIDATE_VERSION_ID
        ...
        issues.add("STEP11_RC0028_RESULT_CONTRACT_MISMATCH")
    if (
    ...
    if (
        ...
        issues.add("STEP11_RC0028_DISPOSITION_INVALID")
    if (
        ...
        issues.add("STEP11_RC0028_FAILURE_CODE_INVALID")
    if (
        ...
        issues.add("STEP11_RC0028_GATE_FAILURE_COUNTS_INVALID")
    numeric_values = (
    ...
    if value.disposition == "selected":
        if (
            ...
            issues.add("STEP11_RC0028_SELECTED_DISPOSITION_INVALID")
        ...
        if (
            ...
            issues.add("STEP11_RC0028_NO_VALID_DISPOSITION_INVALID")
        ...
        if (
            ...
            issues.add("STEP11_RC0028_FAIL_CLOSE_DISPOSITION_INVALID")
    commitments = (
    ...
    if value.result_sha256 != expected_sha256 or not _valid_sha256(
        ...
        issues.add("STEP11_RC0028_RESULT_HASH_MISMATCH")
    return tuple(sorted(issues))


def validate_step11_rc0028_experiment_private_execution(
    value: Any,
) -> tuple[str, ...]:
    """Validate request-local linkage without projecting private material."""

    if type(value) is not Step11Rc0028ExperimentPrivateExecution:
        return ("STEP11_RC0028_PRIVATE_EXECUTION_TYPE_INVALID",)
    issues: set[str] = set()
    if (
        value.schema_version
        != STEP11_RC0028_EXPERIMENT_PRIVATE_EXECUTION_SCHEMA
        ...
        issues.add("STEP11_RC0028_PRIVATE_EXECUTION_CONTRACT_MISMATCH")
    body_free_issues = validate_step11_rc0028_experiment_result(
        value.body_free_result
    )
    if body_free_issues:
        issues.add("STEP11_RC0028_PRIVATE_BODY_FREE_RESULT_INVALID")
        return tuple(sorted(issues))
    ...
    if (
        ...
        issues.add("STEP11_RC0028_PRIVATE_SOURCE_LINKAGE_MISMATCH")
    if value.selection_result is not None and tuple(
        getattr(value.selection_result, "gate_results", ())
    ) != value.gate_results:
        issues.add("STEP11_RC0028_PRIVATE_SELECTION_LINKAGE_MISMATCH")
    ...
    if result.disposition != "fail_close" and (
        ...
        issues.add("STEP11_RC0028_PRIVATE_INVERSE_CARDINALITY_MISMATCH")
    if result.disposition == "selected":
        ...
        if (
            ...
            issues.add("STEP11_RC0028_PRIVATE_SELECTED_LINKAGE_MISMATCH")
    elif any(
        item is not None
        for item in (
            value.selected_final_utf8_bytes,
        ...
        issues.add("STEP11_RC0028_PRIVATE_UNSELECTED_BODY_FORBIDDEN")
    return tuple(sorted(issues))


def step11_rc0028_experiment_result_material(
    value: Step11Rc0028ExperimentBodyFreeResult,
) -> dict[str, Any]:
    """Return the validated shareable receipt without any body-bearing value."""

    issues = validate_step11_rc0028_experiment_result(value)
    if issues:
        raise Step11Rc0028ExperimentRuntimeError(issues[0])
    return {**_result_payload(value), "result_sha256": value.result_sha256}


validate_step11_rc0028_experiment_runtime_result = (
    validate_step11_rc0028_experiment_result
)
step11_rc0028_experiment_runtime_result_material = (

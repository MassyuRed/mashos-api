...
import emlis_nls_v3_rc0029_surface_repair_bounded_experiment as bounded
...
def _case_row(
    ...
    *,
    ...
    if selected:
        ...
        closed_codes = ["STEP11_RC0029_PIPELINE_FAIL_CLOSE"]
    return {
        "case_id": case_id,
...
def _e4_receipt(selected_ids: Iterable[str]) -> dict[str, Any]:
    ...
    repair = _load(_SURFACE_REPAIR_MANIFEST)
    commitment_by_id = {
    ...
    return {
        ...
        "source_dependency_closure_sha256": repair[
...
def test_rc0029_e4_runner_requires_external_product_read_authorization(
    tmp_path: Path,
) -> None:
    runner = getattr(bounded, "run_rc0029_frozen100", None)
    assert callable(runner), "RC0029_E4_RUNNER_REQUIRED"
    ...
    assert captured.value.code == (
        "RC0029_E3_PRODUCT_READ_AUTHORIZATION_REJECTED"
    )
    assert tuple(output_directory.iterdir()) == ()


def test_rc0029_e4_viability_requires_100_accounted_and_selected_above_56() -> None:
    validate_receipt = getattr(
        bounded,
        "validate_rc0029_experiment_body_free_receipt",
        None,
    )
    validate_viability = getattr(
        bounded,
        "validate_rc0029_e4_viability",
        None,
    )
    assert callable(validate_receipt), "RC0029_BODY_FREE_VALIDATOR_REQUIRED"
    assert callable(validate_viability), "RC0029_E4_VIABILITY_VALIDATOR_REQUIRED"
...
def test_rc0029_e4_rejects_old56_regression_or_no_net_new_selection(
...
def test_rc0029_e4_new_selection_is_outside_representative_eight() -> None:
    selected = {*_RC0027_SELECTED_56, _NEW_NON_REPRESENTATIVE_CASE}
...
def test_rc0029_e4_receipt_cannot_claim_formal_acceptance() -> None:
    validate_receipt = getattr(
        bounded,
        "validate_rc0029_experiment_body_free_receipt",
        None,

...
def test_rc0029_four_default_owner_prefixes_are_exact_rc0027_bytes() -> None:
    for name, (size, expected) in _PREFIXES.items():
        source = (_SERVICES / name).read_bytes()
        assert len(source) > size, "RC0029_ADDITIVE_OWNER_SECTION_MISSING"
        assert _sha256(source[:size]) == expected, (
            "RC0027_DEFAULT_OWNER_PREFIX_CHANGED:" + name
        )


def test_rc0029_shared_runtime_and_catalog_are_byte_immutable() -> None:
...
def test_rc0029_shared_runtime_import_is_successor_disconnected() -> None:
    ...
    assert not (_DISCONNECTED_MODULES & frozenset(sys.modules)), (
        "RC0029_SHARED_RUNTIME_REVERSE_IMPORT_DETECTED"
    )

...
"""E3 machine gate for the frozen rc0029 representative eight.
...
_REPRESENTATIVE = _CYCLE_ROOT / "rc0029_representative8_body_free.json"
...
def representative_authority() -> tuple[
    ...
    repair_manifest = _load_json(_SURFACE_REPAIR_MANIFEST)
    closure = repair_manifest.get("source_dependency_closure_sha256")
...
def test_rc0029_e3_fixture_is_exact_body_free_authority(
    representative_authority: tuple[
        dict[str, Any], dict[str, dict[str, Any]], str
    ],
) -> None:
...
def test_rc0029_e3_public_runtime_accounts_for_eight_selected_results(
    ...
    for row in representative_machine_results:
        assert validate_step11_rc0029_experiment_result(row) == ()
...
def test_rc0029_e3_machine_receipts_are_body_free_and_not_product_read(
    representative_machine_results: tuple[Any, ...],
) -> None:
    materials = tuple(
        step11_rc0029_experiment_result_material(row)
        for row in representative_machine_results
    )
    assert all(
